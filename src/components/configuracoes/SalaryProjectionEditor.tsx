...
import { projectSalaryForMonth, sumRevenueLast12Months, calcAnexoVTaxCents } from "@/lib/domain/salary";
...
export function SalaryProjectionEditor({ entries: initialEntries, person }: { entries: SalaryEntry[]; person: Person }) {
  ...
  const projection = useMemo(() => {
    ...
    return months.map((m) => {
      ...
      const taxCents = calcAnexoVTaxCents(grossCents, rbt12Cents);
  ...
  return (
    <Card className="p-5">
      ...
      <p className="mb-4 text-xs text-(--color-text-tertiary)">
        ...
        útil). O imposto usa a alíquota efetiva do Simples Nacional — Anexo V — sobre a receita acumulada dos
        últimos 12 meses.
      </p>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-xs text-(--color-text-tertiary)">
            ...
            <th className="pb-2 font-medium">Imposto (Simples V)</th>
